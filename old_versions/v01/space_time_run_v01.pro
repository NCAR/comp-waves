pro space_time_run, date, velocity, angle, index,outpath, nwlst, freqst, name_addon, $
	                   debug=debug, wr_speeds=wr_speeds,npt_init=npt_init,$
	                   r_inner=r_inner,r_upper=r_upper
...
;        nwlst - Number of wavelength positions in scan as string
;        freqst - Central position of frequency filter as string
...
;                            be around 21. Signal only correlates well over sort distances. 
;		          r_inner - lower boundary of mask defined as occulting disk + some pixels + max offset from correlation
;		          r_upper - upper boundary of mask
;                
;
;CALLS: compute_speed.pro (version modified by R Morton)
;
;
;HISTORY: Created ? ?
;         R Morton 11/2014 - Fixed minor bug for dealing with tracks that are shorter than specified track length.
;                            Turned continue's to break's - if track hits boundary it stops
;                            Added extra comments to code.
;         R Morton 03/2016 - Added if statements to debug only sections of code - hopefully reduce run time.
...
;                            Applied Median smoothing of wave angle map to reduce noise
;
...
norm_cadence=index.norm_cadence
xscale=index.xscale

IF NOT keyword_set(r_inner) THEN r_inner=index.lower_r+index.maxoffset
IF NOT keyword_set(r_upper) THEN r_upper=index.upper_r
mask=index.mask
...
IF n_elements(npt_init) eq 0 THEN npt_init=25 ;number of steps to map along track (make odd)
...
;mask[where(rad lt r_inner)]=0.
;mask[where(rad gt r_upper)]=0.
  ...
  FOR ix=xstart,xend DO BEGIN
      ...
      IF mask[ix,iy] NE 0. THEN BEGIN ;changed from angle
		 ...
		 ;this condition gives the lower limit of the track
	  	 IF sqrt( (float(ix)-nx/2.+0.5)^2 + (float(iy)-ny/2.+0.5)^2 )-imid LT r_inner THEN $
		 ...
		 ;##############################################################
		  ...
		  ;r=compute_speed_new(pro_vel,xscale,norm_cadence,no_tr=no_tr)
...
save, file=outpath+'speeds_'+name_addon+date+'_'+nwlst+'_'+freqst+'.sav',pro_speed,ret_speed, phase_speed,$
      phase_speed_err,pro_speed_err,ret_speed_err,phase_speed_err,pro_power,ret_power,sign_power ;,pro_no_tr,ret_no_tr

if wr_speeds eq 1 then begin
 save,file=outpath+'speeds_testing_'+name_addon+date+'_'+nwlst+'_'+freqst+'.sav',p_vel, r_vel
endif
