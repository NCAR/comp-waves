...
;                 cutflag - specifies which quantity to cut-out. Can be an array! 
;                           0 - Intensity
;			     1 - Line width
;			     2 - Velocity
...
;         Removed USM and RD cuts - added additionaly functionality to allo
;         cuts of anything to be taken
...
PRO CoMPspline,indir=indir,pickpath=pickpath,cutsize=cutsize,jump=jump,cutflag=cutflag
...
;location of saved spline data & make
...
CoMPFDcube=Cdataloc+['Intensity.fcube','Width.fcube','Velocity.fcube','eih','PhaseSpeed.fcube']
CoMPFDcubeI=Cdataloc+'times.sav'
CoMPScube=Cdataloc+['intensity_loop.fcube','width_loop.fcube','velocity_loop.fcube','eih','PhaseSpeed_loop.fcube']	   
...
IF n_elements(cutflag) EQ 0 THEN cutflag=[0,1,2]
...
IF LOOPTRACKER THEN BEGIN
  !p.background=255
 cflen=n_elements(cutflag)
 FOR kk=0,cflen-1 DO BEGIN
     q=cutflag[kk]
     ...
     CASE q OF
      ...
      IF kk EQ 0 THEN p=define_path(pextra=pextra,x0c=x0c,y0c=y0c,dx=dx,dy=dy)
      ...
      slices=dblarr(2*cutsize-1,ds[3]+1,nsamples)   ;define the place we will store the data
      ...
      FOR j=0,n_elements(y0c)-jump-1,jump DO BEGIN 
          ...
          nu=cutx[1]-cutx[0]-2
          slices[*,0,tot]=tempo[0:nu,1]	    ; original intensity variation

          IF q LT 4 THEN BEGIN  
     ...
     ENDIF ELSE BEGIN
        Icube=subdir+'rotTDR'+'_Intenscube_'+compdat+string(jump,format='(I1)')+'.sav'
        save, slices,filename= Icube
     ENDELSE
 ENDFOR
ENDIF
!p.background=0
