...
PRO error_on_fits,date,dv=dv,dw=dw,di=di,see=see
...
FOR i=0,nt-1 DO BEGIN
   ...
   ;Get rid of values lt 0
    ...
    ;Error on intensity values 0.005*875.^2
    IF n_elements(see) GT 0 THEN BEGIN
       delI1=sqrt(I1+bck1+2.*sig_r^2+sobel(I1)^2*see)/sqrt(frame_av)
       delI2=sqrt(I2+bck2+2.*sig_r^2+sobel(I2)^2*see)/sqrt(frame_av)
       delI3=sqrt(I3+bck3+2.*sig_r^2+sobel(I3)^2*see)/sqrt(frame_av)
    ENDIF ELSE BEGIN
    ...
    dvsq=(del*dvdI1*delI1)^2+(del*dvdI2*delI2)^2+(del*dvdI3*delI3)^2

    ;Get rid of -NaN's
    dvsq[where(finite(dvsq,/nan,sign=-1))]=0.
    ...
    dwsq=(dwdI1*delI1)^2+(dwdI2*delI2)^2+(dwdI3*delI3)^2
    
    ;Get rid of -NaN's
    dwsq[where(finite(dwsq,/nan,sign=-1))]=0.
    ...
    disq=(ic*didI1*delI1)^2+(exp(v^2/w^2)*didI2*delI2)^2+(ic*didI3*delI3)^2

    ;Get rid of -NaN's
    disq[where(finite(disq,/nan,sign=-1))]=0.
